...
function getToolLabel(toolName: string, args: Record<string, unknown>): string {
  ...
  if (toolName === "str_replace_editor") {
    switch (args?.command) {
      ...
      case "str_replace": return `Updated ${path}`;
      case "insert":      return `Updated ${path}`;
      case "view":        return `Viewed ${path}`;
      default:            return `Edited ${path}`;
    }
  }
...
export function MessageList({ messages, isLoading }: MessageListProps) {
  if (messages.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full px-4 text-center pt-24">
